...
void printVirMemorySimulationResults(MemorySimulationResults simResults,
	...
	printf("%-30s %llu\n",
			 "Pages Available To User:", simResults.pagesAvaibletoUser);
	...
	for (i = 0; i < numProcesses; i++) {
		...
		printf("\n    Used Page Table Entries: %d (%.2f %%)",
				 ...
				 (processes[i]->numPagesAtTermination / (float)MAX_VIRTUAL_PAGES) *
					  100);
...
void printCacheSimulationResults(CacheSimulationResults results,
											int pageFaults, CacheOutput cacheCalc) {
	int totalMisses;
	int totalHits;
	double hitRate;
	double missRate;
	...
	totalMisses = results.compulsoryMisses + results.conflictMisses +
	...
	if (results.totalInstructions > 0) {
		cpi = ((double)results.totalCycles +
				(pageFaults * 100.0)) / results.totalInstructions;
	} else {
		cpi = 0.0;
	}
	...
	if (cacheCalc.implementation_memory_size_kb > 0) {
		unusedPercent =
			 (unusedKB / cacheCalc.implementation_memory_size_kb) * 100.0;
	} else {
		unusedPercent = 0.0;
	}
	...
	printf("%-24s %.2f KB / %.2f KB = %.2f%%  Waste: $%.2f/chip\n",
			 ...
			 cacheCalc.implementation_memory_size_kb, unusedPercent, wasteCost);

	printf("%-24s %d / %llu\n", "Unused Cache Blocks:", unusedBlocks,
