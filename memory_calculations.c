...
int calculate_memory(int physical_memory_mb, double os_percent,
							int num_trace_files, MemoryCalculationResults *results) {
	...
	if (results == NULL)
		return -1;
	if (physical_memory_mb == 0 || num_trace_files == 0)
		return -2;
	if (os_percent < 0.0 || os_percent > 100.0)
		return -3;
	if (!is_power_of_two_ull((unsigned long long)physical_memory_mb))
		return -4;
	...
	if (!is_power_of_two_ull(number_physical_pages))
		return -5;

	physical_page_bits = log2_ull(number_physical_pages);

	page_table_entries_per_process = 1ULL
												<< (USER_VIRTUAL_ADDRESS_LIMIT_BITS - 12U);

	results->page_table_entry_size_bits = VALID_BIT_COUNT + physical_page_bits;

	total_ram_for_page_tables_bits =
		 page_table_entries_per_process * (unsigned long long)num_trace_files *
	...
	number_pages_for_system =
		 (unsigned long long)((os_percent / 100.0) *
									 (double)number_physical_pages);
